...
ui <- fluidPage(
  ...
  titlePanel("Southern California Cap and Trade System"),
...
server <- function(input, output, session) {
  ...
  marketCurves <- reactive({
    ...
    # Sort facilities by abatement cost to create supply curve
    buyers <- facilities %>% 
    ...
    sellers <- facilities %>%
      filter(net_position > 0) %>%
      arrange(desc(abatement_cost))
    
    # Build demand curve (buyers' willingness to pay)
    if (nrow(buyers) > 0) {
      demand_qty <- cumsum(abs(buyers$net_position))
      demand_price <- buyers$abatement_cost
      demand_df <- data.frame(quantity = demand_qty, price = demand_price)
    } else {
      demand_df <- data.frame(quantity = 0, price = ceilingPrice)
    }
    
    # Build supply curve (sellers' asking price)
    if (nrow(sellers) > 0) {
      supply_qty <- cumsum(sellers$net_position)
      supply_price <- seq(reservePrice, 
                          reservePrice + 20, 
                          length.out = nrow(sellers))
      supply_df <- data.frame(quantity = supply_qty, price = supply_price)
    } else {
      supply_df <- data.frame(quantity = 0, price = reservePrice)
    }
  ...
  output$marketPlot <- renderPlot({
    ...
    ggplot() +
      ...
      annotate("text", x = max(curves$demand$quantity) * 0.95, 
