...
ui <- fluidPage(
  ...
  titlePanel("Southern California Cap and Trade System v2"),
...
server <- function(input, output, session) {
  ...
  marketCurves <- reactive({
    ...
    # Each buyer will pay up to their abatement cost
    buyers <- facilities %>% 
      filter(net_position < 0) %>%
      arrange(desc(abatement_cost))  # Highest willingness to pay first
    
    # Build demand curve (step function)
    if (nrow(buyers) > 0) {
      demand_df <- data.frame(
        quantity = c(0, cumsum(abs(buyers$net_position))),
        price = c(buyers$abatement_cost[1], buyers$abatement_cost)
      )
    } else {
    ...
    # Sellers will sell at or above the reserve price
    # Their asking price increases with quantity (they prefer keeping some allowances)
    sellers <- facilities %>%
      filter(net_position > 0) %>%
      arrange(net_position)  # Smallest surplus first (highest asking price)
    
    # Build supply curve (step function starting at reserve price)
    if (nrow(sellers) > 0) {
      # Sellers ask for increasing prices as they sell more
      # Price starts at reserve and increases based on their abatement cost
      asking_prices <- reservePrice + (sellers$abatement_cost - min(sellers$abatement_cost)) * 0.3
      
      supply_df <- data.frame(
        quantity = c(0, cumsum(sellers$net_position)),
        price = c(asking_prices[1], asking_prices)
      )
    } else {
      # No supply available - vertical line at zero quantity
      supply_df <- data.frame(quantity = c(0, 0), price = c(reservePrice, ceilingPrice))
    }
  ...
  output$marketPlot <- renderPlot({
    ...
    ggplot() +
      ...
      annotate("text", x = max_qty * 0.95, 
               y = input$reservePrice, label = "Reserve Price", 
               color = "darkgreen", vjust = -0.5, size = 3.5) +
      annotate("text", x = max_qty * 0.95, 
               y = input$ceilingPrice, label = "Price Ceiling", 
               color = "darkred", vjust = -0.5, size = 3.5) +
