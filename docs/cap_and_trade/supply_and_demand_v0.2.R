...
# Define UI
ui <- fluidPage(
  titlePanel("Marc's Supply & Demand Interactive Model (v. 0.2)"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      wellPanel(
        ...
        tags$ul(
          tags$li(strong(style = "color: #10b981;", "Demand Curve:"), 
...
server <- function(input, output, session) {
  ...
  output$supply_demand_plot <- renderPlot({
    ...
    p <- ggplot(data, aes(x = quantity)) +
      ...
      scale_color_manual(values = c("Demand" = "#10b981", "Supply" = "#007bff")) +
      labs(
