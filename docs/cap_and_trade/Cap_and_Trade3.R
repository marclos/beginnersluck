...
server <- function(input, output, session) {
  ...
  firmData <- reactive({
    ...
    # Each firm has different willingness to pay based on their business model
    # High-cost firms willing to pay more, low-cost firms willing to pay less
    firms <- firms %>%
      mutate(
        costLevel = c(1.2, 0.9, 1.1, 0.7, 1.0),  # Cost multipliers for each firm
        willingnessToPayMax = minP + (maxP - minP) * costLevel,
        netPosition = allocated - desired,
        action = ifelse(netPosition > 0, "Seller", "Buyer"),
        # Buyers will pay up to their WTP, sellers accept market price
        effectivePrice = ifelse(netPosition < 0, 
                                pmin(willingnessToPayMax, eqPrice), 
                                eqPrice),
        # Only buyers with WTP >= market price will participate
