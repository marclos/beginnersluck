...
ui <- fluidPage(
  ...
  titlePanel("Marc's Cap and Trade Model (v 1.0)"),
...
server <- function(input, output, session) {
  ...
  marketData <- reactive({
    ...
    # Supply: P = minP + (maxP - minP) * (Q / cap)^1.5
    # Demand: P = maxP - (maxP - minP) * (Q / dem)^0.7
    # Solve for Q where supply price = demand price
    
    # Use numerical search to find intersection
    findIntersection <- function() {
      quantities <- seq(0, min(cap, dem), by = 0.1)
      
      supplyPrices <- minP + (maxP - minP) * (quantities / cap)^1.5
      demandPrices <- maxP - (maxP - minP) * (quantities / dem)^0.7
      
      # Find where curves are closest (intersection point)
      differences <- abs(supplyPrices - demandPrices)
      minDiffIndex <- which.min(differences)
      
      list(
        quantity = quantities[minDiffIndex],
        price = (supplyPrices[minDiffIndex] + demandPrices[minDiffIndex]) / 2
      )
    }
    
    intersection <- findIntersection()
    eqQty <- intersection$quantity
    eqPrice <- intersection$price
  ...
  marketCurves <- reactive({
    ...
    # Generate quantities up to the maximum of cap or demand
    maxQ <- max(cap, dem)
    ...
    # Supply only exists up to the cap - beyond that, no more permits available
    ...
    # Demand only exists up to total demand - beyond that, no one wants more
    # At the demand limit, price should be at or below the price floor
  ...
  firmData <- reactive({
    ...
    firms <- firms %>%
      mutate(
        ...
        costLevel = costs,
        # Example: Initial allocation affects firm's willingness to pay
        # (wealth effect - firms with more permits can afford higher prices)
        willingnessToPayMax = minP + (maxP - minP) * costLevel * (1 + 0.1 * (allocated / mean(allocated))),
  ...
  output$marketPlot <- renderPlot({
    ...
    ggplot(curves, aes(x = quantity)) +
      geom_line(aes(y = supply, color = "Supply"), linewidth = 1.2, na.rm = TRUE) +
      geom_line(aes(y = demand, color = "Demand"), linewidth = 1.2, na.rm = TRUE) +
      # Vertical line showing the emissions cap
      geom_vline(xintercept = cap, linetype = "dashed", color = "red", linewidth = 1) +
      annotate("text", x = cap, y = max(curves$demand, na.rm = TRUE) * 0.95, 
               label = "Cap", color = "red", hjust = -0.1) +
      # Mark the equilibrium point - FIXED
      annotate("point", 
               x = mktData$equilibriumQuantity, 
               y = mktData$equilibriumPrice,
               color = "purple", 
               size = 4, 
               shape = 19) +
      annotate("text", 
