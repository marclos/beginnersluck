...
server <- function(input, output, session) {
  ...
  marketData <- reactive({
    ...
    if (dem > cap) {
      # Shortage scenario: price rises as permits become scarce
      # The more demand exceeds cap, the higher the price
      scarcity <- dem / cap  # > 1 when shortage exists
      eqPrice <- minP + (maxP - minP) * min(1, (scarcity - 1) / 0.5)
    } else {
      # Surplus scenario: price falls toward floor (or zero)
      # When supply exceeds demand, permits have little value
      utilizationRate <- dem / cap  # < 1 when surplus exists
      eqPrice <- minP * utilizationRate  # Price approaches floor or below
    }
