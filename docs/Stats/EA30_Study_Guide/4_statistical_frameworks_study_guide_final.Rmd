...
```{r  out.width = "30%", eval=TRUE, echo=FALSE, fig.cap="Calystegia purpurata is a species of morning glory known by the common name Pacific false bindweed. It is endemic to California, where it grows in the seaside scrub of the coastline and the chaparral of the coastal and inland valleys."}
...
To get help with R, see [help function](#help) or [help.start function](#help.start)
...
plot(y1 ~ x1, Anscombe, main="Dataset A", pch=20)
  A.lm = lm(y1 ~ x1, Anscombe); abline(coef(A.lm)); 
    f <- summary(A.lm)$fstatistic
    p <- round(pf(f[1],f[2],f[3],lower.tail=F),3)
  lm.sum = summary(A.lm); str(lm.sum)
  sum <- paste0("m= ", round(coef(A.lm)[2], 1), "; r2=", round(lm.sum$r.squared, 3), "; p=", p); sum
  text(4, 9, sum, adj=0)
plot(y2 ~ x2, Anscombe, main="Dataset B", pch=20)
  B.lm = lm(y2 ~ x2, Anscombe); abline(coef(B.lm))
 f <- summary(B.lm)$fstatistic
    p <- round(pf(f[1],f[2],f[3],lower.tail=F),3)
  lm.sum = summary(B.lm); str(lm.sum)
  sum <- paste0("m= ", round(coef(B.lm)[2], 1), "; r2=", round(lm.sum$r.squared, 3), "; p=", p); sum
  text(4, 9, sum, adj=0)

plot(y3 ~ x3, Anscombe, main="Dataset C", pch=20)
  C.lm = lm(y3 ~ x3, Anscombe); abline(coef(C.lm))
      f <- summary(C.lm)$fstatistic
    p <- round(pf(f[1],f[2],f[3],lower.tail=F),3)
  lm.sum = summary(C.lm); str(lm.sum)
  sum <- paste0("m= ", round(coef(C.lm)[2], 1), "; r2=", round(lm.sum$r.squared, 3), "; p=", p); sum
  text(4, 9, sum, adj=0)

plot(y4 ~ x4, Anscombe, main="Dataset D", pch=20)
  D.lm = lm(y4 ~ x4, Anscombe); abline(coef(D.lm))
    f <- summary(D.lm)$fstatistic
    p <- round(pf(f[1],f[2],f[3],lower.tail=F),3)
  lm.sum = summary(D.lm); str(lm.sum)
  sum <- paste0("m= ", round(coef(D.lm)[2], 1), "; r2=", round(lm.sum$r.squared, 3), "; p=", p); sum
  text(8, 9, sum, adj=0)
```
...
- Syntax: abline(a = NULL, b = NULL, h = NULL, v = NULL, reg = NULL,
       coef = NULL, untf = FALSE, ...)

## aov() 

- Fit an analysis of variance (ANOVA) model by a call to lm for each stratum.
...
- generic function which extracts model coefficients from objects returned by modeling functions. coefficients is an alias for it.
- Syntax: coef(object, ...)

## chisq.test() 

- performs chi-squared contingency table tests and goodness-of-fit tests.
- Syntax: chisq.test(x, y = NULL) (y is ignored if x is a matrix)
...
## file.choose() {#filechose}

- Choose a file interactively.
- Syntax: file.choose(new = FALSE)

## glm() {#glm}

- used used to fit generalized linear models, specified by giving a description of the linear predictor and a description of the error distribution. 

- Syntax: glm(formula, family = gaussian, data, weights, subset,
...
- the primary interface to the help systems. Can be abreviated as ? or ?? for packages.
...
- adds a small amount of noise to a numerical vector. It can help the data look a bit clearer on the graph. 
...
- Syntax: lines(x, y = NULL, type = "l", ...)

## - lm() 
- used to fit linear models, including multivariate ones. It can be used to carry out regression, single stratum analysis of variance and analysis of covariance (although aov may provide a more convenient interface for these)
-Syntax: lm(formula, data, subset, weights, na.action,
...
## par() {#par} 
- used to set or query graphical parameters. Parameters can be set by specifying them as arguments to par in tag = value form, or by passing them as a list of tagged values.
- Syntax: par(..., no.readonly = FALSE)

## points() 

- draw points at specific coordinates and to be plotted in a plotting structure, a two-column matrix, or a time series. 
...
- Syntax: power.anova.test(groups = NULL, n = NULL,
                 between.var = NULL, within.var = NULL,
                 sig.level = 0.05, power = NULL)

## predict() {#predict} 

- generic function for predictions from the results of various model fitting functions. The function invokes particular methods which depend on the class of the first argument.

## read.csv() 

- Reads a comma separated file in table format and creates a data frame from it, with cases corresponding to lines and variables to fields in the file. 

- Syntax: dataframe.name <- read.csv("input.csv")

- To use this, we first need to import the csv file into r. Then use file.choose() to figure out the path. Once the path is figured out, you can use the function to bring the data into R and do fun stuff!
...
- uses cross-classifying factors to build a contingency table of the counts at each combination of factor levels.
