...
```{r out.width = "40%", eval=TRUE, fig.cap="A deciduous climbing or trailing evergreen shrub.
Native blackberry is not aggressive like it’s Himalayan cousin. The berries are edible and tasty. Flowers host beneficial insects. Tolerates salt spray in shoreline and bluff plantings"}
...
Calochortus: The rcode is using the lm [linear regression model command] to create a line of best fit and illustrate the relationship between between SRP and HABs.
...
```{r  out.width = "50%", eval=TRUE, fig.cap="Calochortus plummerae is a perennial herb (bulb) that is native to California, and endemic (limited) to California."}
knitr::include_graphics("https://github.com/marclos/beginnersluck/raw/master/Stats/EA30_Study_Guide/type-i-ii-error.png") 
```
