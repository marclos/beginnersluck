...
<<echo=FALSE, fig=TRUE>>=
par(cex=1.7, las=1)
hist(N1000, main=NA, xlab="Values", xlim=c(-2,5), ylim=c(0,250), breaks=20, col='gray')
#box()
@
...
These data will represent the population (N) and has a mean or $\mu$ of \Sexpr{round(mean(N1000), 4)}. The values range from \Sexpr{round(range(N1000)[1],2)} to \Sexpr{round(range(N1000)[2],2)} (Figure~\ref{fig:hist_pop}). From this dataset, we sample 10 numbers randomly. 
...
n10
@ 
\noindent and a mean or $\bar{y}$ = \Sexpr{round(mean(n10),3)}. These values certainly fall within the range of values in the population.

\subsection{R's Default Boxplot}

\begin{figure}
\caption{Default Boxplot where the following are shown: median =\Sexpr{round(median(n10),2)}; Range = \Sexpr{round(range(n10), 2)}; and interquartile range = \Sexpr{round(quantile(n10)[c(2,4)], 2)}.}
\label{fig:default_bp}
<<fig=T, echo=F>>=
par(las=1, cex=2)
boxplot(n10, col="yellow", ylim=c(-1, 3))
@
\end{figure}
...
\noindent (\Sexpr{round(var(n10), 3)}) and the standard deviation (\Sexpr{round(sd(n10), 3)}), which is the square root of $s^2$. Then, we can calculate the standard error of the sample using the following equation: 
...
Sorted.csv = "/home/CAMPUS/mwl04747/github/beginnersluck/Confidence_Intervals/2019_EA30F_Waste_Audit_Sorted.csv"
# Read Raw Data
...
boxplot(Percent ~ Type, data=droplevels(Sorted[Sorted$Trash_Recycle=="R",]))
boxplot(Percent ~ Type, data=droplevels(Sorted[Sorted$Trash_Recycle=="T",]))
@
\end{figure*}
