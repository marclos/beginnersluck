---
title: "Covid-19 Analyses for Los Angles"
...
```{r, echo=FALSE, caption='Confirm Cases in Los Angeles County'}
...
```{r echo=FALSE, results='hide', fig.cap='Predicted Confirmed Cases in Los Angeles', warning=FALSE, message=FALSE}
...
*Weekend values are not updated on github site for LA County. Thus the estimates are lower on the weekends because values have not been updated.
...
In the USA in general, we have about 2.1 beds per 1,000 people in the state of California. With approximately 10.6 million residents, there are approximately `r (totalbeds=2.1/1000*10.6e6)` beds in the county. However, they are generally not sitting around empty. In normal times there is a 50% occupancy rate. There is no reason to believe this is a normal time, and to guess if that rate is higher or lower is anyone's guess. Nevertheless, for the sake exploring the implications, let's estimate the number of empty beds in LA County based on these assumptions and `r (beds= totalbeds*.50)` beds exists in LA County.  
...
test = data.frame(Date = as.Date("2020-03-28"), Confirmed=1804); test
