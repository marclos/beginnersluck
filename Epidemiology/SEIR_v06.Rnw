...
N = c(50, 100, 100, 100); 
Tstep = c(2000, 2000, 3000, 3000)
velocity = c(5, 5, 5, 5)
#Tstep = c(50, 50, 50, 50) #4380; #Six months, hourly steps.
stationary = c(0, 0, .4, .4) # as a percent of N
infect_dist = c(20, 20, 20, 20)
asymptomatic = c(24, 24, 24, 60) # period of time of being exposed
symptomatic = c(96, 96, 96, 60) # time after exposure
recovery = 720 # 30 days to become uninfectious
init_infect = c(2, 2, 2, 2) # Initial number of infections
...
<<>>=
MoveSubjects = function(N=3, tstep=4, Velocity = 5, Stationary = 0, Infect_dist = 5, Init_infect=1, Speed=5){
# Test Defaults
...
m=1
tic("Model1")
Model1<- MoveSubjects(N = params[m,2], Velocity = params[m, 3], 
    tstep = params[m,4], Stationary = params[m,5], 
    Infect_dist=params[m, 6], Init_infect=params[m, 7])
toc() # 900s

m=2
tic("Model2")
Model2<- MoveSubjects(N = params[m,2], Velocity = params[m, 3], 
    tstep = params[m,4], Stationary = params[m,5], 
    Infect_dist=params[m, 6], Init_infect=params[m, 7])
toc() #1800s
...
m=3
tic("Model3")
Model3 <- MoveSubjects(N = params[m,2], Velocity = params[m, 3], 
    tstep = params[m,4], Stationary = params[m,5], 
    Infect_dist=params[m, 6], Init_infect=params[m, 7])
toc() #3308
...
m=4
tic("Model4")
Model4 <- MoveSubjects(N = params[m,2], Velocity = params[m, 3], 
    tstep = params[m,4], Stationary = params[m,5], 
    Infect_dist=params[m, 6], Init_infect=params[m, 7])
toc() #3308
@
...
plot(S~Days, data=results, ty='l', col=SEIRcol[1], ylab="", ylim=c(0,params[m,2]), xlab="Days", xlim=c(0,tstep/24), lwd=2, main = paste(params[m, 1], ", Stationary =", params[m,4]*100, "%"))
